...
- Follow-up with an animated canvas MediaStream reached `boxing.mirror.detector_ready`, displayed the upper-body framing cue, kept the stream live, produced no page errors, and disposed its track to `ended`. The synthetic blank scene appropriately produced no punch estimates. Screenshot: `/tmp/boxing-mirror.png` (local QA artifact, not a learner recording).
