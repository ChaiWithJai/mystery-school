...
export class SchoolWorld{
  constructor(container,labels,onSelect){
    this.container=container;this.labelContainer=labels;this.onSelect=onSelect;this.random=rng(4271);this.paused=matchMedia('(prefers-reduced-motion: reduce)').matches;this.selected=null;this.lens='together';this.blender=false;
    ...
    this.makeIslands();this.makeAtmosphere();this.makeConnections();this.addLabels();this.resize();new ResizeObserver(()=>this.resize()).observe(container);
  ...
  animate(){requestAnimationFrame(()=>this.animate());const t=this.clock.getElapsedTime();this.camera.position.lerp(this.targetPosition,.035);this.look.lerp(this.targetLook,.04);this.camera.lookAt(this.look);if(!this.paused){this.dust.rotation.y=t*.006;this.islands.forEach((g,i)=>{g.position.y=(i===0?.4:Math.sin((i-1)*2)*.25)+Math.sin(t*.45+i)*.07;});this.portalMeshes.forEach(o=>{if(o.userData.curve){o.position.copy(o.userData.curve.getPoint((o.userData.phase+t*.06)%1));}else{o.rotation.y=Math.sin(t*.5)*.1;}});}
    const w=this.container.clientWidth,h=this.container.clientHeight;this.labels.forEach(({button,group,index})=>{const p=group.getWorldPosition(new THREE.Vector3()).add(new THREE.Vector3(0,index===-1?4.2:2.05,0)).project(this.camera);button.style.left=`${(p.x*.5+.5)*w}px`;button.style.top=`${(-p.y*.5+.5)*h}px`;button.style.visibility=p.z>1||p.x<-1.1||p.x>1.1?'hidden':'visible';});this.renderer.render(this.scene,this.camera);}
}
