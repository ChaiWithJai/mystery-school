...
export function mirrorPosition(points,t){
  ...
  return {t:Math.max(0,Math.round(t)),x:clamp((points[23].x+points[24].x)/2),y:clamp((points[23].y+points[24].y)/2),ankles};
}
...
export function mountBoxingMirror(container,{initialState={},onChange=()=>{},onEvent=()=>{},lessons=[]}={}) {
  ...
  function process(points,t,origin){
    const position=mirrorPosition(points,t-roundStarted);
    if(activeRound&&position&&t-lastPositionAt>=100){activeRound.points.push(position);lastPositionAt=t;}

    canvas.width=video.videoWidth;canvas.height=video.videoHeight;ctx.clearRect(0,0,canvas.width,canvas.height);ctx.lineWidth=3;
    if(activeRound?.points.length){ctx.save();ctx.strokeStyle='#f2d58f';ctx.shadowColor='#f2d58f';ctx.beginPath();activeRound.points.forEach((p,i)=>{const method=i?'lineTo':'moveTo';ctx[method](p.x*canvas.width,p.y*canvas.height);});ctx.stroke();const last=activeRound.points.at(-1);for(const a of last.ankles){ctx.beginPath();ctx.arc(a.x*canvas.width,a.y*canvas.height,7,0,Math.PI*2);ctx.stroke();}ctx.restore();}
    ctx.strokeStyle='#b7f5bb';ctx.shadowColor='#99efa6';ctx.shadowBlur=10;
    for(const [a,b]of BONES){if(!points?.[a]||!points?.[b]||(points[a].visibility??1)<.5||(points[b].visibility??1)<.5)continue;ctx.beginPath();ctx.moveTo(points[a].x*canvas.width,points[a].y*canvas.height);ctx.lineTo(points[b].x*canvas.width,points[b].y*canvas.height);ctx.stroke();}
    if(!points||[11,12,13,14,15,16,23,24].some(i=>!points[i]||(points[i].visibility??1)<.5)){status.textContent=position?(lessons.find(l=>l.id===state.lessonId)?.cues?.[0]||'Notice where you move before and after the punch.'):'Bring your hips into view to trace your movement.';return;}
