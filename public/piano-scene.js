...
export function mountPianoScene(container,adapter,track,onContinue){
  ...
  next.onclick=async()=>{piano.querySelector('[data-stop]').click();next.disabled=true;next.textContent='Keeping your take...';try{await onContinue?.();}catch(error){const message=piano.querySelector('[data-error]');message.hidden=false;message.textContent='Could not keep this take: '+error.message;}finally{next.disabled=false;next.textContent='Keep this. Into the ring →';}};
  ...
  const roll=mountPianoRoll(piano,{adapter,soundtrack,onEvent:track,getPlaybackClock:()=>adapter.getPlaybackClock?.()});
