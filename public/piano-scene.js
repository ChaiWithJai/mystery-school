...
import {RUNAWAY_OPENING_SOURCE,createOpeningDemoTake,createOpeningPresenterTake,analyzeOpening,openingTempoLabel} from './runaway-opening.js';
...
export function mountPianoScene(container,adapter,track,onContinue){
  ...
  sync();return{render:sync,event(type,payload){event(type,payload);roll.event(type,payload);},dispose(){disposed=true;piano.removeEventListener('presenter-opening',presenterOpening);soundtrack.leavePiano();roll();rollStyles.remove();scene.remove();}};
}
