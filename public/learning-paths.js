  ...
  async function open(pathway,artifactId,deposit=null){
    ...
    q('[data-imagine]').onclick=()=>{
      ...
      experimentDispose=mountLearningExperiment(q('.learning-next'),{
        ...
        getContext:()=>({question:draft.question.trim(),explanation:draft.explanation,source_refs:structuredClone(currentSources()),reference_draft:[draft.lab.youtubeUrl||'',draft.lab.youtubeTimestamp||'',draft.lab.youtubeNote||''],story_writing:[draft.lab.story_world?.learnerIntent||'',draft.lab.story_world?.learnerStory||'']}),
        getState:()=>structuredClone(draft.lab),
        setState:value=>{if(typeof dispose?.setState!=='function')throw Error('This experiment cannot apply changes yet.');if(boxingGame&&value.boxing_round){boxingGame.setState(value.boxing_round);draft.lab.boxing_round=boxingGame.getState();}if(worldReflection&&value.real_world_reflection)worldReflection.setState(value.real_world_reflection);dispose.setState(value);if(storyWorld)storyWorld.setState({...storyWorld.getState(),modelComparison:value.modelComparison,decisionResponses:value.decisionResponses,storyChoice:value.storyChoice});},
        save:()=>save('new_question',actorKind,draft.question),
        track:(type,payload)=>{if(pathway==='movement'&&['learning.experiment.preview','learning.experiment.apply'].includes(type)){const affected=q('.universe-physics');if(affected){affected.classList.add('projection-game');affected.hidden=false;affected.open=true;}}return events.track(type,payload);}
      });
      q('[data-imagine]').hidden=true;
    };
    try {
      const module=await import(person.module);if(token!==opening)return;
      validateVideo=module.validateYouTubeReference||null;
      sources=module.IDEAS_SOURCES||[{label:person.domain+' diagram',url:location.origin+'/'+person.module.slice(2),locator:'Implemented mathematical model; not a real-world measurement'}];
      q('[data-lab]').replaceChildren();
      dispose=module[person.mount](q('[data-lab]'),{
