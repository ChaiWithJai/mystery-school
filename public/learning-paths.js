...
const PATHS={
  music:{name:'Maya',age:15,domain:'Music / calculus',goal:'Find and play the opening of Runaway, then make a musical idea of my own.',moment:'Maya can find a melody by ear. She has been playing the same unfinished phrase for her grandmother, who keeps asking to hear the ending.',voice:'I know how I want it to feel. I do not know how to make the first note gentler.',care:'We can stay with your song. Try the sound first. If its beginning feels too sudden, we can look at how quickly its loudness rises.',share:'Could the opening arrive more gently, while keeping the tune I recognize?',peer:'Grandmother',next:'What do I hear differently after trying the opening myself?',module:'./song-lab.js',mount:'mountSongLab'},
  movement:{name:'Andre',age:19,domain:'Movement / physics',goal:'Watch a boxing lesson, see myself try it, and notice what I want to change.',moment:'Andre watches his training partner move with control. He wants to understand what changes during a reach, but remembers being told he was behind in maths.',voice:'Can we start with the movement? I can show you what I mean before I know the words.',care:'Yes. Follow the marked point. Keep the distance the same and give the movement more time. We can name the relationship after you notice it.',share:'Can you show the same distance taking longer, and point to where the motion is fastest?',peer:'Training partner',next:'How would a different movement change the shape of the graph?',module:'./movement-lab.js',mount:'mountMovementLab'},
...
export function createLearningPaths({openDrawer,body,onCleanup,api,sessionId,track,toast,onImagine,onOpen,onArtifactSaved}){
  ...
  async function open(pathway,artifactId){
    ...
    let experimentDispose=null,musicScene=null,pianoScene=null,boxingGame=null,boxingMirror=null,worldReflection=null;
    const cleanup=()=>{opening++;boxingMirror?.dispose();worldReflection?.dispose();boxingGame?.dispose();pianoScene?.dispose();musicScene?.dispose();experimentDispose?.();dispose?.();dispose=null;document.querySelector('#drawer').classList.remove('learning-drawer');};
    onCleanup(cleanup);
    ...
    if(ready&&pathway==='movement'){
      ...
      boxingMirror=mountBoxingMirror(mirrorHost,{initialState:draft.lab.boxing_mirror||{},lessons:BOXING_LESSONS,onChange:state=>{draft.lab.boxing_mirror=structuredClone(state);keep();},onEvent:(type,payload)=>events.track('learning.movement.action',{pathway,actor_kind:actorKind,type,payload})});
    ...
    if(ready&&pathway==='ideas'){
      ...
      leave.onclick=()=>{const showing=host.hidden;host.hidden=!showing;q('.ideas-lab').hidden=showing;leave.textContent=showing?'Return to the story ↩':'Take this into life ↗';events.track('learning.ideas.action',{pathway,actor_kind:actorKind,type:'reflection.open',payload:{open:showing}});};
    }
